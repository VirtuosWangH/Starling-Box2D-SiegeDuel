package scenes
{
    ...
    public class MovieScene extends Scene
    {
        ...
        {
            var frames:Vector.<Texture> = SiegeDuel.assets.getTextures("flight");
            mMovie = new MovieClip(frames, 15);
            
            // add sounds
            var stepSound:Sound = SiegeDuel.assets.getSound("wing_flap");
            mMovie.setFrameSound(2, stepSound);
			
            // move the clip to the center and add it to the stage
            mMovie.x = SDContext.CenterX - int(mMovie.width / 2);
            mMovie.y = SDContext.CenterY - int(mMovie.height / 2);
            addChild(mMovie);
